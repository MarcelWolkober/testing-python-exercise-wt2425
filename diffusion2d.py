...
class SolveDiffusion2D:
    ...
    def initialize_physical_parameters(self, d=4.0, T_cold=300.0, T_hot=700.0):
        assert isinstance(d, float), "Diffusion coefficient must be a float"
        assert isinstance(T_cold, float), "Cold temperature must be a float"
        assert isinstance(T_hot, float), "Hot temperature must be a float"
        self.D = d
        self.T_cold = T_cold
        self.T_hot = T_hot
